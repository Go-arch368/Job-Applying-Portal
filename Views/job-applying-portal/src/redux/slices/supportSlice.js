...
export const sendQuery = createAsyncThunk("/support/sendQuery", async ({ jobId, query }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(err?.response?.data?.error || "Failed to send query")
  }
})

export const getQuery = createAsyncThunk("/support/getQuery", async ({ jobId }, { rejectWithValue }) => {
  try {
    ...
    console.log(err?.response?.data?.error)
    return rejectWithValue(err?.response?.data?.error || "Failed to fetch query")
  }
})

export const recruiterqueries = createAsyncThunk("/support/recruiterqueries", async (_, { rejectWithValue }) => {
  try {
    ...
    console.log(err?.response?.data?.error)
    return rejectWithValue(err?.response?.data?.error || "Failed to fetch recruiter queries")
  }
})

export const responsePost = createAsyncThunk("/support/responsePost", async ({ id, response }, { rejectWithValue }) => {
  try {
    ...
    console.log(err?.response?.data?.error)
    return rejectWithValue(err?.response?.data?.error || "Failed to post response")
  }
})
