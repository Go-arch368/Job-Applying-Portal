...
export const userRole = createAsyncThunk("users/userRole", async (_, { rejectWithValue }) => {
    try {
        ...
        console.log(err)
        return rejectWithValue(err?.response?.data?.error || err?.response?.data || "Failed to authenticate")
    }
})

export const profileUpdate = createAsyncThunk("users/profileUpdate", async (formData, { rejectWithValue }) => {
    try {
        const response = await axios.put("/api/userDetailsPost", formData, {
            headers: {
...
const usersSlice = createSlice({
    ...
    initialState: {
        ...
        user: [],
        serverErrors: null,
        isLoading: true  // start true so we wait for userRole to resolve
    },
    reducers: {
        logout: (state, action) => {
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(userRole.pending, (state) => {
            state.isLoading = true
        })
        builder.addCase(userRole.fulfilled, (state, action) => {
            return { ...state, isloggedIn: true, user: action.payload, serverErrors: null, isLoading: false }
        })
        builder.addCase(userRole.rejected, (state, action) => {
            return { ...state, serverErrors: action.payload, user: null, isLoading: false }
        })
